	...
	stat_helper <- data.frame(stat_helper1=stat_helper1, stat_helper2=stat_helper2, stat_helper3=stat_helper3)
	...
	write("\tchrom_array=$( head -n${run} _helper_admix.txt | tail -n1 | cut -f1 )", file=a.script, append=T)
	write("", file=a.script, append=T) 
	write("\tstart_array=$( head -n${run} _helper_admix.txt | tail -n1 | cut -f2 )", file=a.script, append=T)
	write("", file=a.script, append=T)
	write("\tend_array=$( head -n${run} _helper_admix.txt | tail -n1 | cut -f3 )", file=a.script, append=T)
	write("", file=a.script, append=T)
	
	# add header to output file
	header <- paste('\tgunzip -cd ', project_directory, '/', vcf_header_file, ' | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
